...
class FlightDataValidator:
    ...
    def validate_add_flight_request(request) -> bool:
        ...
        if not any(required_data) not in request.form.keys():
    ...
    def create_chronometer_object(request):
        if FlightDataValidator.validate_add_flight_request(request):
            return Chronometer(
                        instructor=User.query.filter(User.id == request.form.get('instructor')).first(),
                        pilot_passenger=User.query.filter(User.id == request.form.get('pilot_passenger')).first(),
                        start_type=request.form.get('start_type'),
                        winch_operator=User.query.filter(User.id == request.form.get('winch_pilot')).first(),
                        tow_pilot=User.query.filter(User.id == request.form.get('airplane_pilot')).first(),
                        glider=Glider.query.filter(Glider.id == request.form.get('glider')).first(),
                        airplane=Airplane.query.filter(Airplane.id == request.form.get('airplane')).first()
                        )
    ...
    def check_duplicate_ids(flight) -> bool:
        ...
        :return:
        """
        crew = [flight.instructor,flight.pilot_passenger,flight.winch_operator,flight.tow_pilot]
        for member in crew:
